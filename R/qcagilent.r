...
#' @importFrom magrittr %>%
#' @importFrom dplyr select slice inner_join arrange
...
#' @importFrom magrittr %>%
#' @export
qcagilent <- function( files, sif = NULL, arraytype = NULL, path = "." )
{
   path %>% file.path("QCAgilent") -> Path
   Path %>% dir.create
   files %>% agilentrawdataextraction(outputdata=TRUE,arraytype=arraytype,path=Path) -> re0
   re0$m0 -> m0
   re0$m1 -> m1
   #
   # agilent QC control
   #
   #
   # E1A spice pos control
   #
   m0 %>% colnames
   if(arraytype=="rna")
   {
     m0 %>% dplyr::slice(grep("E1A",m0$ProbeName)) -> e1a0
     e1a0 %>% head
     data.frame(
     ...
     e1a1 %>%
       dplyr::inner_join(e1ainfo,by="E1AType") %>%
       dplyr::select(c(1,2,(3+length(files)+1),4:(3+length(files)))) %>%
       dplyr::arrange(E1AConcentrationpgL)  -> e1a2
     ...
     e1a2 %>% moal::output(FileName)
     #
     # E1A plot
     #
     rep(as.character(e1a2$E1AType),dim(e1a2)[2]-3) -> color
     # joining dot in the plot
     rep(paste("e1a",1:dim(e1a2)[1],sep= ""),dim(e1a2 )[2]-3) -> group
     e1a2 %>% dplyr::select(-c(1,2,3)) %>% log2 %>% data.frame %>% stack %>%
       ggplot(aes(x=ind,y=values,color=color,group=group)) +
   ...
   if(is.null(sif))
   {
      ...
      data.frame("SampleID"=colnames(m1)[-1],FileName,stringsAsFactors=F) -> Sif
      paste("metadata_",length(files),".tsv",sep="") -> FileName
      Path %>% file.path(FileName) -> FileName
      Sif %>% moal::output(FileName)
   }else
      {
         paste("metadata_",length(files),".tsv",sep="") -> FileName
         Path %>% file.path(FileName) -> FileName
   ...
   # 3xSLv1
   #
   m0 %>% dplyr::slice(grep("3xSLv1",m0$ProbeName)) -> slv10
   paste("QCneg_3xSLv1_rawdata_",dim(slv10)[2]-1,"_",dim(slv10)[1],".tsv",sep="") -> FileName
   Path %>% file.path(FileName) -> FileName
   slv10 %>% moal::output(FileName)
   # 3xSLv1 plot
   slv10 %>% dplyr::select(-1) %>% data.frame %>% stack %>%
   ...
   # QC
   #
   if(is.null(sif)){ moal::qc(dat=m1,path=Path,dirname="QCrawdata") }else
     { moal::qc(dat=m1,sif=sif,path=Path,dooutputinput=F,dirname="QCrawdata") }
}
