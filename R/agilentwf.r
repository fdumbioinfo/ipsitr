...
#' @importFrom stats sd
#' @export
agilentwf <- function( files , sif = NULL , bgfilter = 16 , dirname = NULL , path = "." )
{
  "agilentwf" -> DirName
  ...
  m0 %>% select(-1) %>% apply(1,mean) %>% ">"(.,bgfilter) %>% which -> sel
  m0[ sel , ] -> m1
  ### normalization
  m1 %>% select(-1) %>% norm(.) -> m2
  ...
  Path %>% file.path( FileName ) -> FileName
  a2 %>% select( .data$ProbeName , .data$Symbol , .data$SystematicName , .data$Description ) %>% output( FileName )
  ### QC normdata
