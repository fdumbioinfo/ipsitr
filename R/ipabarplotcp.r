...
#' @export
ipabarplotcp <- function( dat , top = 80 , labsize = 5 )
{
  ...
  p + ggtitle( "Canonical Pathways (log10pvalFDR)" ) -> p
  ...
  Dat0 %>% ggplot( aes( x= reorder(.data$Name,.data$ENAScore), y = .data$ENAScore ) ) -> p
  p + ggtitle( "Canonical Pathways (Enrichment)" ) -> p
  p + geom_bar( stat = "identity" , position="dodge2" , fill = "#E69F00" ) -> p
  p + coord_flip() -> p
  p + theme_minimal() -> p
  p + theme( 
